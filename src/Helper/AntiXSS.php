...
class AntiXSS
{
    ...
    private function _entity_decode($str)
    {
        ...
        /** @noinspection UsageOfSilenceOperatorInspection */ /** @noinspection PhpUsageOfSilenceOperatorInspection */
        // HHVM dons't support "ENT_DISALLOWED" && "ENT_SUBSTITUTE"
        $flags = Bootup::is_php('5.4') ? ENT_QUOTES | ENT_HTML5 | @ENT_DISALLOWED | @ENT_SUBSTITUTE : ENT_QUOTES;
        ...
        if (preg_match_all('/&[A-Za-z]{2,}[;]{0}/', $str, $matches)) {
            ...
            if (null === $HTML_ENTITIES_CACHE) {
                ...
                $HTML_ENTITIES_CACHE = array_merge($entitiesSecurity, array_flip(get_html_translation_table(HTML_ENTITIES, $flags)), array_flip(self::$entitiesFallback));
            }
    ...
    private function _compact_exploded_javascript($str)
    {
        ...
        foreach ($words as $word) {
            ...
            if (!isset($WORDS_CACHE[$word])) {
                $regex = '(?:\s|\+|"|\042|\'|\047)*';
                $word = $WORDS_CACHE[$word] = substr(chunk_split($word, 1, $regex), 0, -strlen($regex));
            } else {
            ...
            $str = (string) \preg_replace_callback('#(' . $word . ')(\W)#is', array(
                $this,
    ...
    private function _decode_string($str)
    {
        ...
        if (preg_match($regExForHtmlTags, $str, $matches) === 1) {
            $str = (string) \preg_replace_callback($regExForHtmlTags, array(
                $this,
                '_decode_entity',
            ),                                     $str);
        } else {
    ...
    {
        $this->_evil_attributes = array_diff($this->_evil_attributes, array_intersect($strings, $this->_evil_attributes));
    ...
    {
        $this->_evil_html_tags = array_diff($this->_evil_html_tags, array_intersect($strings, $this->_evil_html_tags));
    ...
    private function _remove_disallowed_javascript($str)
    {
        do {
            ...
            if (stripos($str, '<a') !== false) {
                $str = (string) \preg_replace_callback('#<a[^a-z0-9>]+([^>]*?)(?:>|$)#i', array(
                    $this,
            ...
            if (stripos($str, '<img') !== false) {
                $str = (string) \preg_replace_callback('#<img[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#i', array(
                    $this,
            ...
            if (stripos($str, '<audio') !== false) {
                $str = (string) \preg_replace_callback('#<audio[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#i', array(
                    $this,
                    '_js_src_removal_callback',
                ),                                     $str);
            }
            ...
            if (stripos($str, '<video') !== false) {
                $str = (string) \preg_replace_callback('#<video[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#i', array(
                    $this,
                    '_js_src_removal_callback',
                ),                                     $str);
            }
            ...
            if (stripos($str, '<source') !== false) {
                $str = (string) \preg_replace_callback('#<source[^a-z0-9]+([^>]*?)(?:\s?/?>|$)#i', array(
                    $this,
                    '_js_src_removal_callback',
                ),                                     $str);
            }
    ...
    {
        return (string) \preg_replace_callback('#\+([0-9a-zA-Z]+)\-#', array($this, '_repack_utf7_callback'), $str);
    }
    ...
    private function _repack_utf7_callback($strings)
    {
        ...
        $strings = (string) \preg_replace_callback('/^((?:\x00.)*?)((?:[^\x00].)+)/us', array($this, '_repack_utf7_callback_back'), $strTmp);
    ...
    private function _sanitize_naughty_html($str)
    {
        ...
        $str = (string) \preg_replace_callback('#<(/*\s*)(' . $evil_html_tags . ')([^><]*)([><]*)#i', array(
            $this,
    ...
    {
        $str = preg_replace('#(alert|eval|prompt|confirm|cmd|passthru|eval|exec|expression|system|fopen|fsockopen|file|file_get_contents|readfile|unlink)(\s*)\((.*)\)#siU', '\\1\\2&#40;\\3&#41;', $str);
